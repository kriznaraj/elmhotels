...
Elm.HotelsList = Elm.HotelsList || {};
Elm.HotelsList.make = function (_elm) {
   "use strict";
   _elm.HotelsList = _elm.HotelsList || {};
   if (_elm.HotelsList.values) return _elm.HotelsList.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   var backgroundImage = function (url) {
      return $Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "backgroundImage",_1: A2($Basics._op["++"],"url(",A2($Basics._op["++"],url,")"))}
                                            ,{ctor: "_Tuple2",_0: "backgroundRepeat",_1: "no-repeat"}]));
   };
   var hotelCard = function (hotel) {
      return A2($Html.li,
      _U.list([]),
      _U.list([A2($Html.div,
      _U.list([$Html$Attributes.$class("hotel-card")]),
      _U.list([A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.div,_U.list([$Html$Attributes.$class("hotel-image"),backgroundImage(hotel.image)]),_U.list([]))
              ,A2($Html.div,
              _U.list([$Html$Attributes.$class("hotel-overlay")]),
              _U.list([A2($Html.h3,_U.list([$Html$Attributes.$class("truncate")]),_U.list([$Html.text(hotel.name)]))
                      ,A2($Html.div,
                      _U.list([$Html$Attributes.$class("hotel-text")]),
                      _U.list([A2($Html.p,
                              _U.list([$Html$Attributes.$class("hotel-price")]),
                              _U.list([$Html.text(A2($Basics._op["++"],"Price: ",$Basics.toString(hotel.price)))]))
                              ,A2($Html.p,
                              _U.list([$Html$Attributes.$class("hotel-rating")]),
                              _U.list([$Html.text(A2($Basics._op["++"],"Rating: ",$Basics.toString(hotel.rating)))]))
                              ,A2($Html.p,
                              _U.list([$Html$Attributes.$class("hotel-stars")]),
                              _U.list([$Html.text(A2($Basics._op["++"],"Stars: ",$Basics.toString(hotel.stars)))]))]))]))]))]))]));
   };
   var hotelList = function (hotels) {
      return A2($Html.section,
      _U.list([$Html$Attributes.$class("hotel-list")]),
      _U.list([_U.eq($List.length(hotels),0) ? A2($Html.h3,_U.list([]),_U.list([$Html.text("Loading hotels ...")])) : A2($Html.ul,
      _U.list([]),
      A2($List.map,hotelCard,hotels))]));
   };
   var Hotel = F6(function (a,b,c,d,e,f) {    return {name: a,thumbnail: b,image: c,stars: d,rating: e,price: f};});
   return _elm.HotelsList.values = {_op: _op,Hotel: Hotel,backgroundImage: backgroundImage,hotelCard: hotelCard,hotelList: hotelList};
};
Elm.Destination = Elm.Destination || {};
Elm.Destination.make = function (_elm) {
   "use strict";
   _elm.Destination = _elm.Destination || {};
   if (_elm.Destination.values) return _elm.Destination.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
...
Elm.Pager.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var view = F3(function (total,paging,address) {
      ...
      var lastPage = _U.eq(pageNum,pageCount);
      var firstPage = _U.eq(pageIndex,0);
      return A2($Html.section,
      _U.list([$Html$Attributes.$class("pager")]),
      _U.list([A2($Html.button,
              _U.list([$Html$Attributes.$class("button prev")
                      ,$Html$Attributes.disabled(firstPage)
                      ,A2($Html$Events.onClick,address,_U.update(paging,{pageIndex: pageIndex - 1}))]),
              _U.list([$Html.text("Previous")]))
              ,A2($Html.span,
              _U.list([$Html$Attributes.$class("total-pages")]),
              _U.list([$Html.text(A2($Basics._op["++"],
              "Page ",
              A2($Basics._op["++"],$Basics.toString(A2($Debug.watch,"pageNum",pageNum)),A2($Basics._op["++"]," of ",$Basics.toString(pageCount)))))]))
              ,A2($Html.button,
              _U.list([$Html$Attributes.$class("button next")
                      ,$Html$Attributes.disabled(lastPage)
                      ,A2($Html$Events.onClick,address,_U.update(paging,{pageIndex: pageIndex + 1}))]),
              _U.list([$Html.text("Next")]))]));
   });
   var Model = F2(function (a,b) {    return {pageSize: a,pageIndex: b};});
   var initialModel = A2(Model,20,0);
   return _elm.Pager.values = {_op: _op,Model: Model,initialModel: initialModel,view: view};
};
...
Elm.Models.make = function (_elm) {
   ...
   var _op = {};
   var Model = F4(function (a,b,c,d) {    return {hotels: a,total: b,criteria: c,autocompleter: d};});
   var Criteria = F3(function (a,b,c) {    return {filter: a,sort: b,paging: c};});
   var AutocompleterUpdate = function (a) {    return {ctor: "AutocompleterUpdate",_0: a};};
   var SortChange = function (a) {    return {ctor: "SortChange",_0: a};};
   var FilterChange = function (a) {    return {ctor: "FilterChange",_0: a};};
   var PageChange = function (a) {    return {ctor: "PageChange",_0: a};};
...
Elm.Api.make = function (_elm) {
   ...
   var imageUrl = function (estabPart) {
      return A2($Basics._op["++"],"https://d23wf1heedwns1.cloudfront.net/ei",A2($Basics._op["++"],estabPart,"/0_260_260.jpg"));
   };
   var estabIdToImageUrl = function (estabId) {
      return imageUrl(A3($String.foldl,
      F2(function (c,str) {    return A2($String.append,str,A2($Basics._op["++"],"/",$String.fromChar(c)));}),
      "",
   ...
   var hotels = function () {
      ...
      $Json$Decode.succeed(""),
      A2($Json$Decode.map,estabIdToImageUrl,A2($Json$Decode._op[":="],"EstablishmentId",$Json$Decode.$int)),
      A2($Json$Decode._op[":="],"Stars",$Json$Decode.$int),
      A2($Json$Decode._op[":="],"UserRating",$Json$Decode.$float),
      A2($Json$Decode._op[":="],"TeaserPricePerNight",$Json$Decode.$float));
      return A2($Json$Decode._op[":="],"Establishments",$Json$Decode.list(hotel));
   }();
   var destinationToJson = function (dest) {
      ...
      A2($Basics._op["++"],", \"PolygonId\" : ",A2($Basics._op["++"],$Basics.toString(dest.polygonId),", \"PageStrand\" : 1}"))))))))))));
   };
   var parseResponse = function (response) {
      var res = A2($Task.map,function (hl) {    return $Models.LoadData(hl);},A2($Http.fromJson,hotels,response));
      return A2($Task.onError,res,function (err) {    return $Task.succeed($Models.LoadData(_U.list([])));});
   };
   var body = function (dest) {
      ...
      A2($Basics._op["++"],destJson,"}")));
   };
   var hotelsPost = function (dest) {
      return {verb: "POST",headers: _U.list([{ctor: "_Tuple2",_0: "Content-type",_1: "application/json"}]),url: "api/hotels",body: body(dest)};
   };
   var getHotels = function (dest) {    return parseResponse(A2($Http.send,$Http.defaultSettings,hotelsPost(dest)));};
...
Elm.Header.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var header = A2($Html.h1,_U.list([]),_U.list([$Html.text("Travel Republic")]));
   return _elm.Header.values = {_op: _op,header: header};
};
